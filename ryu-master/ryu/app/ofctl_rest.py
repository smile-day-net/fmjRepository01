...
class StatsController(ControllerBase):
    ...
    @command_method
    def mod_flow_entry(self, req, dp, ofctl, bpf_map, cmd, **kwargs):
        cmd_convert = {
        ...
        ofctl.mod_flow_entry(dp, bpf_map, mod_cmd)

    @command_method
    def mod_bpf_map(self, req, dp, ofctl, flow, cmd, **kwargs):
        cmd_convert = {
            'update': dp.ofproto.NXT_BPF_MAP_UPDATE,
        }
        mod_cmd = cmd_convert.get(cmd, None)
        if mod_cmd is None:
            raise CommandNotFoundError(cmd=cmd)

        ofctl.mod_bpf_map(dp, flow, mod_cmd)
...
class RestStatsApi(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        uri = path + '/bpfmap/{cmd}'
        mapper.connect('stats', uri,
                       controller=StatsController, action='mod_bpf_map',
                       conditions=dict(method=['POST']))
