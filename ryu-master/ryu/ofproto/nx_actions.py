...
def generate(ofp_name, ofpp_name):
    ...
    class NXActionExecuteProg(NXAction):
        """
           Execute BPF program action

           This action set BPF program to execute.
           ...
           Example::
            ...
            actions += [parser.NXActionExecuteProg(prog_id=1)]
           """
        _subtype = nicira_ext.NXAST_RAW_EXECUTE_PROG
        ...
        def __init__(self, prog_id,
                     type_=None, len_=None, vendor=None, subtype=None):
            super(NXActionExecuteProg, self).__init__()
            self.prog_id = prog_id

        @classmethod
        def parser(cls, buf):
            (prog_id,) = struct.unpack_from(cls._fmt_str, buf, 0)
            return cls(prog_id)

        def serialize_body(self):
            data = bytearray()
            msg_pack_into(self._fmt_str, data, 0, self.prog_id)
            return data
