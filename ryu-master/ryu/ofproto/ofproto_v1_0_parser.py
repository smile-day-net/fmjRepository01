...
class NXTBpfUpdateMap(NiciraHeader):
    def __init__(self, datapath, prog_id=0, map_id=0, nb_elems=1024, key=None, value_index=0, value=None):
        super(NXTBpfUpdateMap, self).__init__(
            datapath, ofproto.NXT_BPF_UPDATE_MAP)
    ...
    def _serialize_body(self):
        self.serialize_header()
        msg_pack_into(ofproto.NX_BPF_MAP_UPDATE_FORMAT_PACK_STR,
                      self.buf, ofproto.NICIRA_HEADER_SIZE, self.prog_id, self.map_id, self.key_size, self.value_size,
                      self.nb_elems, self.key, self.value_index, self.value)
