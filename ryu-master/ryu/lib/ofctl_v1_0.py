...
def to_actions(dp, acts):
    ...
    for a in acts:
        ...
        if action_type == 'OUTPUT':
            ...
            subtype = a.get('sub_type', 0)
            prog_id = str_to_int(a.get('prog_id', 0))
            if subtype == 'NXAST_RAW_EXECUTE_PROG':
                actions.append(dp.ofproto_parser.NXActionExecuteProg(prog_id))
            else:
                LOG.error('Unknown nicira action subtype')
        else:
...
def mod_bpf_map(dp, bpf_map, cmd):
    prog_id = str_to_int(bpf_map.get('prog_id', 0))
    map_id = str_to_int(bpf_map.get('map_id', 0))
    nb_elems = UTIL.ofp_buffer_from_user(
        bpf_map.get('nb_elems', 1))
    key = socket.htonl(ipv4_to_int(bpf_map.get('key', None)))
    value_index = socket.htonl(str_to_int(bpf_map.get('value_index', 0)))
    value = socket.htonl(ipv4_to_int(bpf_map.get('value', 0)))

    bpf_map_mod = dp.ofproto_parser.NXTBpfUpdateMap(datapath=dp,
                                                    prog_id=prog_id, map_id=map_id, nb_elems=nb_elems,
                                                    key=key, value_index=value_index,
                                                    value=value)

    ofctl_utils.send_msg(dp, bpf_map_mod, LOG)
